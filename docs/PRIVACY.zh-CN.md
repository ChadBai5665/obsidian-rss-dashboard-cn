...
插件可能请求你配置的 RSS、Atom、JSON Feed、播客 Feed 或网站，进行 Feed 自动发现，或获取你明确打开、保存的单条网页全文。YouTube 日常采集会请求频道/视频 Feed 和元数据；只有明确获取字幕时，内置路径才请求 YouTube 当前公开提供的公开人工字幕或自动生成字幕。标题和简介不会被表示为字幕；没有公开字幕轨道时会显示“无字幕”，不会拼造正文。可选 TikHub 只用于 X 账号/主题；可选 AI 只在你手动点击单条 AI 操作时访问所选服务商，AI 点击本身不会触发全文或字幕抓取。
...
AI 启动输入固定为 `fetchFullText=false`。选择器优先使用已经缓存的文章全文或 YouTube 字幕，但不会因为点击 AI 操作而抓取全文，也不会同时请求字幕。若希望模型处理全文或字幕，应先在阅读区明确使用独立的“获取全文”或“获取字幕”，等待缓存成功后再发起 AI 操作。没有缓存时，受限快照只使用当前单条信息已有的标题、来源名称/地址、正文片段或简介及操作指令。
