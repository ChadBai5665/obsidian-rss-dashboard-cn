...
| `.rss-dashboard-data/state/operation-journal/YYYY-MM-DD.jsonl` | 按本机日期保存的字幕、AI、刷新和订阅管理运行记录 |
...
信息台“运行记录”会把同一次操作的事件按稳定的本地操作 ID 聚合，默认显示最近 7 天，也可以切换到最近 30 天。覆盖范围只有四类实际后台工作：字幕获取、手动 AI 操作、订阅刷新，以及订阅的新增、修改、暂停、恢复和删除。普通的展开、收起、滚动和筛选切换不会写入记录。

本地记录只保存受限字段，例如时间、类别、动作、手动/启动/定时触发方式、阶段、状态、本地项目或来源 ID、最多 200 个字符的本地显示名称，以及受限的计数、耗时、服务商/模型、标准错误码和保存结果。它不记录 API Key、Token、Cookie、Authorization 请求头、完整 URL、凭证文件路径、字幕正文、文章正文、AI 输入、AI 输出、提示词、原始接口响应或原始错误对象。

TikHub 相关字段只表达本机证据：

- `confirmedPaidRequests`：本机已经确认并计入预算账本的请求数，不等同于服务商最终扣费数。
- `possiblySent`：请求已经越过本机可确认的发送边界；如果随后超时，只能说明可能已经发送，不能推断 TikHub 是否完成处理，也不能据此安全自动重发。
- 本地 `jobId`：只在 TikHub 明确返回任务 ID 后写入本地记录，用于关联后续免费轮询；脱敏导出只保留 `jobIdPresent=true`，不会导出任务 ID 的值。

这些字段不是 TikHub 官方账单。端点价格、计费规则、服务端执行状态和最终费用均以 TikHub 官方信息与账单为准。

运行记录默认保留最近 30 个自然日，总容量最多 10MB。系统先清理超过保留期的合法日志，再在容量超限时从最旧日期开始清理；不会把未知文件或其他状态文件当成运行记录删除。末行不完整、中间行损坏、写入或维护失败时，界面会标记“不完整”“损坏”或相应健康状态，并继续显示能够安全读取的合法事件；日志故障不改变字幕、AI、刷新或订阅操作本身的结果。

“导出脱敏记录”先提供预览。导出会移除本地标题或来源名称、完整 URL、文件路径、TikHub 任务 ID、字幕/文章正文、AI 输入与输出、提示词、凭证和原始错误；本地项目/来源 ID 只变成不可逆的短哈希关联值。即使如此，公开分享前仍应人工检查预览。

“清空记录”只删除受控目录中的运行记录日期文件，不删除配置、密钥、订阅、字幕、采集内容与历史、AI 分析或任何用户 Markdown。日志相对路径仅用于高级排障；普通用户不需要打开或手工编辑 JSONL 文件，应优先使用信息台和设置中的查看、导出与清空入口。
