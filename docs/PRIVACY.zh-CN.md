...
插件可能请求你配置的 RSS、Atom、JSON Feed、播客 Feed 或网站，进行 Feed 自动发现，或获取你明确打开、保存或交给 AI 的单条网页全文。YouTube 采集会请求频道/视频 Feed 和元数据；插件不下载字幕、音频或视频。可选 TikHub 只用于 X 账号/主题，可选 AI 只在你确认单条操作后访问所选服务商。
...
- 发现目录内记录的第三方图片地址；
- 打开 YouTube 播放器时的 `youtube-nocookie.com` iframe。

这些请求可能向图片主机、Google、YouTube 或其他第三方暴露你的 IP 地址、来源域名或目标域名。`youtube-nocookie.com` 是隐私增强嵌入域，但打开播放器仍会产生第三方请求，并不等于离线播放。
