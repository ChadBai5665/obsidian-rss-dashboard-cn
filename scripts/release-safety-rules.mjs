const SENSITIVE_KEY_SOURCE =
  String.raw`(?:api[\s_-]*(?:key|token)|access[\s_-]*(?:key|token)|client[\s_-]*secret|refresh[\s_-]*token|private[\s_-]*key|auth(?:orization|[\s_-]*token)?|token|secret|password|session(?:[\s_-]*(?:id|key|token))?|key)`;
