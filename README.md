...
发起 AI 操作时固定使用 `fetchFullText=false`：选择器会优先使用已经缓存的文章全文或 YouTube 字幕，但不会因为点击 AI 操作而抓取全文，也不会顺带获取字幕。如果希望模型处理这些内容，请先在阅读区使用独立的“获取全文”或“获取字幕”操作，等待缓存成功后再发起 AI 操作；没有缓存时只使用当前单条信息已有的标题、来源元数据、正文片段或简介。快照不会扫描并发送其他订阅、其他文章、其他笔记或整个知识库。
