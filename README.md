...
字幕是另一项明确操作。内置路径只尝试 YouTube 当前公开提供的公开人工字幕或自动生成字幕，优先人工原语言轨道；标题和简介不会被表示为字幕，也不会用来填补字幕正文。如果视频没有任何公开字幕轨道，界面会显示“无字幕”，本版本不伪造字幕。内置路径明确确认没有公开字幕后，可选回退不可用或失败也会保持“无字幕”状态。YouTube 的未公开接口形状可能变化，插件不承诺每个公开视频都能持续取得字幕；“无字幕”和“暂时不可用”会分别提示。
