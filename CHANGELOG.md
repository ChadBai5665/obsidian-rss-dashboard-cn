...
- 在信息台增加本地“运行记录”主视图，可按最近 7 天或最近 30 天查看字幕、AI、订阅刷新和订阅管理的聚合时间线。
- 在“设置 → 导入与导出 → 诊断信息”增加运行记录统计、脱敏预览/复制和受控清空入口。
- 运行记录按天保存在当前知识库的数据目录中，默认保留 30 天且总容量最多 10MB；部分损坏或写入失败会显示健康提示，不改变原业务结果。

### Privacy and scope

- 运行记录不包含凭证、完整 URL、凭证路径、字幕或文章正文、AI 输入/输出、提示词、原始接口响应或原始错误。
- TikHub 记录只提供本机已确认请求数、请求可能已发送和是否收到任务 ID 的证据；实际费用仍以 TikHub 官方账单为准。
- 脱敏导出会移除标题、URL、路径、任务 ID、正文、AI 输出和凭证；“清空记录”只删除运行记录，不删除配置、密钥、订阅、字幕、采集历史、AI 分析或 Markdown。
...
- AI 连接编辑弹窗在窄窗口中保持在可视边界内，并改用响应式表单布局。
- 可在 AI 连接弹窗中使用当前填写的模型和密钥进行测试；测试不会自动保存密钥。
- 草稿内容变更后旧测试结果立即失效；未结束的测试会继续占用全局测试锁，避免重复计费请求。
- 连接名称、模型 ID 或接口地址校验失败时，不再清空尚未保存的 API 密钥。
