...
export class RssDashboardView extends ItemView {
  ...
  public actionFocusReader(): void {
    ...
    if (!readerLeaf) {
      new Notice(this.t("dashboard.noReaderPane"));
      return;
    }
  ...
  // --- Render pipeline ---
  onOpen(): Promise<void> {
    this.collectionViewDisposed = false;
      ...
      this.articleList = new ArticleList(
        ...
        this.selectedArticle,
        {
          ...
          onMarkAllAsUnread: () => {
            ...
            if (read.length === 0) {
              new Notice(this.t("dashboard.noReadCurrent"));
              return;
            }
            void this.plugin
  ...
  /**
   * Loads the entire local-day snapshot before any collection filter is
   * applied. The generation guard prevents an asynchronously completed read
   * from painting a dashboard that has since been closed or replaced.
   */
  public async loadCollectionItems(): Promise<void> {
    ...
    try {
      ...
      if (
        ...
        generation !== this.collectionLoadGeneration
      ) {
        return;
      }
  ...
  public setCollectionSection(section: CollectionSection): void {
    this.collectionSection = section;
    this.render();
  }

  public setCollectionQueryText(text: string): void {
    this.collectionQueryText = text;
    this.render();
  }

  /** Returns an objectively filtered collection snapshot, never a ranked list. */
  public getCollectionSectionItems(): CollectedItem[] {
    ...
    return this.collectionQueryService.query(input);
  }

  private renderCollectionSections(container: HTMLElement): void {
    ...
    section.createDiv({
      ...
      text: this.t("dashboard.collection"),
    });

    const controls = section.createDiv({
      cls: "rss-dashboard-collection-controls",
    });
    ...
    for (const option of labels) {
      const button = controls.createEl("button", {
        text: option.label,
        cls: "rss-dashboard-collection-section-button",
        attr: { type: "button" },
      });
      button.toggleClass(
        "is-active",
        option.section === this.collectionSection,
      );
      button.addEventListener("click", () =>
        this.setCollectionSection(option.section),
      );
    }

    const collapse = controls.createEl("button", {
      text: this.collectionCollapsed
        ? this.t("dashboard.expandCollection")
        : this.t("dashboard.collapseCollection"),
      cls: "rss-dashboard-collection-collapse",
      attr: {
        type: "button",
        "aria-expanded": String(!this.collectionCollapsed),
      },
    });
    ...
    const refreshAll = controls.createEl("button", {
      text: this.t("dashboard.refreshAll"),
    ...
    const refreshFailed = controls.createEl("button", {
      text: this.t("dashboard.refreshFailed"),
      cls: "rss-dashboard-collection-refresh-failed",
      attr: { type: "button" },
    });
    ...
    const search = section.createEl("input", {
      cls: "rss-dashboard-collection-search",
      attr: {
        type: "search",
        placeholder: this.t("dashboard.searchCollected"),
        "aria-label": this.t("dashboard.searchCollected"),
      },
    });
    ...
    const read = filterControls.createEl("select", {
      ...
      attr: { "aria-label": this.t("dashboard.readState") },
    });
    for (const option of [
      { value: "all", label: this.t("common.all") },
      { value: "read", label: this.t("common.read") },
      { value: "unread", label: this.t("common.unread") },
    ]) {
      read.createEl("option", { value: option.value, text: option.label });
    }
    read.value =
      this.collectionRead === true
        ? "read"
        : this.collectionRead === false
          ? "unread"
          : "all";
    ...
    if (this.collectionLoadError) {
      ...
      results.createDiv({ text: this.t("dashboard.collectionLoadFailed") });
      return;
    }
  ...
  private renderCollectionResults(results: HTMLElement): void {
    ...
    for (const item of items) {
      ...
      row.createSpan({
        cls: "rss-dashboard-collection-item-title",
        text: item.title,
      });
      row.createSpan({
        cls: "rss-dashboard-collection-content-basis",
      ...
      const source = row.createEl("button", {
        cls: "rss-dashboard-collection-source-refresh",
        text: item.sourceName,
        attr: {
          type: "button",
          title: this.t("dashboard.refreshSource"),
          "aria-label": `${this.t("dashboard.refreshSource")}: ${item.sourceName}`,
        },
      });
  ...
  private runCollectionRefreshAction(action: () => Promise<void>): void {
    void action().catch(() => {
      ...
      new Notice(this.t("dashboard.sourceRefreshFailed"));
    });
  }
  ...
  private renderFilterSubheader(container: HTMLElement): void {
    ...
    if (hasKeywordStats) {
      ...
      const filterEditBtn = filterStatsRow.createEl("button", {
        ...
        attr: {
          type: "button",
          title: this.t("dashboard.editKeywordRules"),
          "aria-label": this.t("dashboard.editKeywordRules"),
        },
      });
    ...
    if (hasHighlightStats) {
      ...
      const highlightEditBtn = highlightRow.createEl("button", {
        ...
        attr: {
          type: "button",
          title: this.t("dashboard.editHighlights"),
          "aria-label": this.t("dashboard.editHighlights"),
        },
      });
      ...
      highlightRow.createSpan({
        ...
        text: this.t("dashboard.highlights"),
      });
    ...
    if (hasDashboardMultiFilterStats && this.dashboardMultiFilterCounts) {
      ...
      const viewFiltersBtn = viewingFilterRow.createDiv({
        ...
        attr: {
          role: "button",
          tabindex: "0",
          title: this.t("dashboard.openViewingFilters"),
          "aria-label": this.t("dashboard.openViewingFilters"),
        },
      });
      ...
      viewingFilterRow.createSpan({
        ...
        text: hasActiveDashboardMultiFilters
          ? this.t("dashboard.viewingFilters", { shown, filteredOut, total })
          : this.t("dashboard.noFilters", { shown, filteredOut, total }),
      });
    }
    ...
    const applyCollapsedState = () => {
      ...
      toggleButton.setAttribute(
        "aria-label",
        this.isFilterSubheaderCollapsed
          ? this.t("dashboard.expandFilterStatus")
          : this.t("dashboard.collapseFilterStatus"),
      );
      toggleButton.setAttribute(
        "aria-expanded",
  ...
  private getArticlesTitle(): string {
    if (this.currentFeed) {
      ...
      return this.t("dashboard.starredItems");
    } else if (this.currentFolder === "unread") {
      return this.t("dashboard.unreadItems");
    } else if (this.currentFolder === "read") {
      return this.t("dashboard.readItems");
    } else if (this.currentFolder === "saved") {
      return this.t("dashboard.savedItems");
    } else if (this.currentFolder === "videos") {
      return this.t("filter.videos");
      ...
      return this.t("dashboard.allArticles");
    }
  }
  ...
  private getActiveFeedRulesForScope(articles: FeedItem[]): Array<{
    ...
    for (const article of articles) {
      ...
      result.push({
        feedTitle: feed.title || this.t("dashboard.untitledFeed"),
  ...
  private buildKeywordFilterTooltip(
    ...
    if (bypassAll) {
      lines.push(this.t("dashboard.keywordBypassEnabled"));
      lines.push("");
    }

    if (globalRules.length > 0) {
      lines.push(
        this.t("dashboard.keywordGlobalRules", {
          logic: globalIncludeLogic,
        }),
      );
    ...
    if (feedRules.length > 0) {
      lines.push(this.t("dashboard.keywordFeedRules"));
      feedRules.forEach((entry) => {
        lines.push(
          `- ${this.t("dashboard.keywordFeedRule", {
            feed: entry.feedTitle,
            logic: entry.includeLogic,
          })}`,
        );
  ...
  private formatRuleForTooltip(rule: KeywordFilterRule): string {
    return this.t("dashboard.keywordRule", {
      type: this.t(
        rule.type === "include"
          ? "dashboard.keywordTypeInclude"
          : "dashboard.keywordTypeExclude",
      ),
      keyword: rule.keyword.trim(),
      mode: this.t(
        rule.matchMode === "exact"
          ? "dashboard.keywordModeExact"
          : "dashboard.keywordModePartial",
      ),
      locations: this.formatRuleLocations(rule),
    });
  }
  ...
  private formatRuleLocations(rule: KeywordFilterRule): string {
    ...
    if (rule.applyToTitle) {
      parts.push(this.t("dashboard.keywordLocationTitle"));
    }
    if (rule.applyToSummary) {
      parts.push(this.t("dashboard.keywordLocationSummary"));
    }
    if (rule.applyToContent) {
      parts.push(this.t("dashboard.keywordLocationContent"));
    }
    if (rule.applyToURL) {
      parts.push(this.t("dashboard.keywordLocationUrl"));
    }
    return parts.join(", ");
  }
  ...
  private async openArticleInNewTab(
    ...
    const resolvedContext =
      contentContext ?? (await this.resolveReaderContentContext(article));
    ...
    if (leaf) {
      ...
      if (leaf.view instanceof ReaderView) {
        ...
        if (resolvedContext) {
          await view.displayItem(article, relatedItems, resolvedContext);
        } else {
  ...
  private openArticleInExternalBrowser(article: FeedItem): void {
    ...
    if (!url) {
      new Notice(this.t("dashboard.noExternalUrl"));
      return;
    }
  ...
  private async resolveReaderContentContext(
    article: FeedItem,
  ): Promise<ReaderContentContext | undefined> {
    const itemId = article.rssDashboardId;
    if (!itemId) return undefined;

    try {
      const collected = await this.plugin.getCollectedItemById(itemId);
      if (collected) {
        return { contentBasis: collected.contentBasis };
      }
    } catch {
      // The already-loaded daily snapshot remains a durable read fallback if
      // a fresh repository lookup is temporarily unavailable.
    }

    const visibleRecord = this.collectionItems.find((item) => item.id === itemId);
    return visibleRecord
      ? { contentBasis: visibleRecord.contentBasis }
      : undefined;
  }
  ...
  private async markPageArticlesAsRead(
    ...
    if (updatedArticles.length === 0) {
      new Notice(this.t("dashboard.noUnreadPage"));
      return;
    }
    ...
    if (!pageCompositionChanged && !paginationChanged) {
      ...
      this.refreshFilterStatusBarOnly();
      new Notice(
        this.t("dashboard.markedRead", { count: updatedArticles.length }),
      );
      return;
    }
  ...
  private getViewFilterReasonLabel(): string | null {
    const specialFolderLabels: Record<string, string> = {
      unread: this.t("dashboard.filterReasonUnread"),
      read: this.t("dashboard.filterReasonRead"),
      starred: this.t("dashboard.filterReasonStarred"),
      saved: this.t("dashboard.filterReasonSaved"),
      videos: this.t("dashboard.filterReasonVideos"),
      podcasts: this.t("dashboard.filterReasonPodcasts"),
    };
    ...
    if (
      ...
      const statusLabels: Record<string, string> = {
        unread: this.t("common.unread"),
        read: this.t("common.read"),
        starred: this.t("navigation.starred"),
        saved: this.t("navigation.saved"),
      };
      return this.t("dashboard.filterReasonStatus", {
    ...
    if (
      ...
      return this.t("dashboard.filterReasonTag", { tag: tagFilter });
    }

    if (this.activeStatusFilters.size > 0 || this.activeTagFilters.size > 0) {
      return this.t("dashboard.filterReasonCurrent");
    }

    if (this.selectedTags.length === 1) {
      return this.t("dashboard.filterReasonTag", {
        tag: this.selectedTags[0],
      });
    }

    if (this.selectedTags.length > 1) {
      return this.t("dashboard.filterReasonCurrentTags");
    }
  ...
  private async handleUpdateFeed(feed: Feed): Promise<void> {
    try {
      ...
      new Notice(
        this.t("dashboard.updateFeedFailed", {
          feed: feed.title,
          error:
            error instanceof Error
              ? error.message
              : this.t("dashboard.unknownError"),
        }),
      );
    }
  }
  ...
  private updateRefreshButtonText(): void {
    ...
    let refreshText = this.t("dashboard.refreshAllFeeds");

    if (this.currentFeed) {
      refreshText = this.t("dashboard.refreshFeed", {
        feed: this.currentFeed.title,
      });
    } else if (
      this.currentFolder &&
      ...
      refreshText = this.t("dashboard.refreshFeedsInFolder", {
        count: feedsInFolder.length,
      ...
      refreshText = this.t("dashboard.refreshFeedsWithTags", {
        count: feedsWithTags.length,
        mode,
        tags: this.selectedTags.join(", "),
      });
    } else {
      refreshText = this.t("dashboard.refreshAllFeedCount", {
        count: this.settings.feeds.length,
      });
    }
  ...
  private async openArticleInConfiguredReaderLocation(
    ...
    if (podcastPlayingLeaves.length > 0) {
      ...
      if (targetLeaf && !podcastPlayingLeaves.includes(targetLeaf)) {
        ...
        await this.openArticleInSpecificLeaf(article, targetLeaf, contentContext);
        return;
      }

      const newLeaf = await this.openArticleInNewTab(article, contentContext);
  ...
  public async openSavedArticleFile(
    ...
    try {
      ...
      if (!leaf) {
        throw new Error(this.t("reader.noSavedLeaf"));
      }
